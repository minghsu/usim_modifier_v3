...
class arr(base_plugin):
    ...
    def execute(self, arg_components: components, arg_arguments=''):
        ...
        uicc_resp: uicc_sel_resp = uicc.select(UICC_FILE.MFARR)
        if uicc_resp.sw1 == 0x90:
            print("EF_ARR ({})".format(UICC_FILE.MFARR))
            for i in range(uicc_resp.count):
                resp = uicc.read_record(i+1, uicc_resp)
                if resp != None:
                    print("#{}: {}".format(i+1, toHexString(resp)))
        else:
            print(self.get_res("read_error").format(UICC_FILE.MFARR))

        print("")

        uicc_resp: uicc_sel_resp = uicc.select(UICC_FILE.ADFARR)
        if uicc_resp.sw1 == 0x90:
            print("EF_ARR ({})".format(UICC_FILE.ADFARR))
            for i in range(uicc_resp.count):
                resp = uicc.read_record(i+1, uicc_resp)
                if resp != None:
                    print("#{}: {}".format(i+1, toHexString(resp)))
        else:
            print(self.get_res("read_error").format(UICC_FILE.ADFARR))
